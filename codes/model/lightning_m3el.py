...
from codes.model.modeling_m3el import M3ELEncoder, M3ELMatcher

class LightningForM3EL(pl.LightningModule):
    def __init__(self, args):
        super(LightningForM3EL, self).__init__()
        self.args = args
        self.save_hyperparameters(args)

        self.encoder = M3ELEncoder(args)
        self.matcher = M3ELMatcher(args)
    ...
    def training_step(self, batch):
        ...
        mention_text_embeds, mention_image_embeds, mention_text_seq_tokens, mention_image_patch_tokens = self.encoder(**mention_batch)
        entity_text_embeds, entity_image_embeds, entity_text_seq_tokens, entity_image_patch_tokens = self.encoder(**ent_batch)

        if 'WikiDiverse' in self.args.data.kb_img_folder:
            logits, (text_logits, image_logits, text_image_logits, image_text_logits), (cl_loss) = self.matcher(entity_text_embeds,
                                                                                                                ...
                                                                                                                train_flag=True, bidirection=True)
            ...
            text_image_loss = self.loss_fct(text_image_logits, labels)
            image_text_loss = self.loss_fct(image_text_logits, labels)
            overall_loss = self.loss_fct(logits, labels)
            if cl_loss is not None:
                loss = overall_loss + text_loss + image_loss + text_image_loss + image_text_loss + cl_loss
            else:
                loss = overall_loss + text_loss + image_loss + text_image_loss + image_text_loss
        else:
            logits, (text_logits, image_logits, image_text_logits), (cl_loss) = self.matcher(entity_text_embeds,
                                                                                             entity_text_seq_tokens,
                                                                                             mention_text_embeds,
                                                                                             mention_text_seq_tokens,
                                                                                             entity_image_embeds,
                                                                                             ...
                                                                                             mention_image_embeds,
                                                                                             mention_image_patch_tokens,
                                                                                             train_flag=True, bidirection=False)
            labels = torch.arange(len(mention_text_embeds)).long().to(mention_text_embeds.device)
            text_loss = self.loss_fct(text_logits, labels)
            image_loss = self.loss_fct(image_logits, labels)
            image_text_loss = self.loss_fct(image_text_logits, labels)
            overall_loss = self.loss_fct(logits, labels)
            if cl_loss is not None:
                loss = overall_loss + text_loss + image_loss + image_text_loss + cl_loss
            else:
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for idx in range(math.ceil(self.args.data.num_entity / chunk_size)):
            ...
            chunk_score, _, _ = self.matcher(chunk_entity_text_embeds, chunk_entity_text_seq_tokens,
                                          mention_text_embeds, mention_text_seq_tokens,
                                          chunk_entity_image_embeds, chunk_entity_image_patch_tokens,
                                          mention_image_embeds, mention_image_patch_tokens, train_flag=False)
    ...
    def on_validation_start(self):
        ...
        with torch.no_grad():
            for batch in tqdm(entity_dataloader, desc='UpdateEmbed', total=len(entity_dataloader), disable=True):
